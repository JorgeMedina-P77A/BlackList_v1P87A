...
public class PhoneCallReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, final Intent intent) {
        ...
        try {
            ...
            if (intent.getAction().equals("android.intent.action.PHONE_STATE")) {
                ...
                DaoListaNegra dao = new DaoListaNegra(context);

                String numberCall = intent.getStringExtra(TelephonyManager.EXTRA_INCOMING_NUMBER);

                POJO_ListaNegra obj = dao.buscarTel(numberCall);


                if (obj.getBlock_tel() == 1) {

                    disconnectPhoneItelephony(context);

                    if (a == 0) {

                        DaoHistorial call = new DaoHistorial(context);

                        if(call.insert(new POJO_Historial(0,numberCall,"Bloqueado",0,"",fecha()+" - "+horasistema()))>0) {

                            Toast.makeText(context, "Se ha bloqueado: "+numberCall, Toast.LENGTH_SHORT).show();

                        }else{

                            Toast.makeText(context, "Problema al registrar bloqueo", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(context,"PHONECALLRECEIVER - "+err.getMessage(),Toast.LENGTH_LONG).show();
    ...
    private void disconnectPhoneItelephony(Context context) {
        try {
            ...
            Toast.makeText(context,"ITELEPHONY"+e.getMessage(),Toast.LENGTH_LONG).show();
        }
    }
